...
export const metadata: Metadata = {
  title: "Verkäufer-Bestellungen – Project Atlas",
};
...
export default async function SellerOrdersPage() {
  ...
  return (
    <main className="mx-auto flex max-w-4xl flex-col gap-8 px-4 py-16 sm:px-6 lg:px-8">
      <h1 className="text-3xl font-bold tracking-tight">Verkäufer-Bestellungen</h1>

      {orders.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          Es liegen noch keine Verkäufe deiner Angebote vor.
        </p>
      ) : (
              ...
              <CardHeader className="flex flex-col gap-3 border-b pb-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex flex-col items-start gap-2 sm:items-end">
                  ...
                  <Button asChild variant="outline" size="sm">
                    <Link href={`/seller/orders/${order.orderId}`}>Versand verwalten</Link>
                  </Button>
                </div>
              </CardHeader>
